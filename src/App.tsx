...
function App() {
  ...
  const selectMaterial = (key: CaseMaterialKey, file: File | null) => {
    ...
    change(s => ({ ...invalidateCase(s), sources: Object.fromEntries(Object.entries(s.sources).filter(([, source]) => source.material !== key)) }));
  };
  const analyze = async () => {
    ...
    for (const key of selected) {
      ...
      try {
        const pages = await readMaterial(material.file!);
        if (epoch !== analysisEpoch.current) return;
        const candidates = candidatesFromPages(pages, key, material.name);
        const blank = pages.filter(p => !p.text).length;
        setMaterials(s => ({ ...s, [key]: { ...material, pages, candidates, status: "ready", message: `已读取${pages.length}个文本位置，找到${candidates.length}项候选。${blank ? `其中${blank}页无文字，需对照扫描页人工检查。` : "请对照原件核对文字完整性。"}` } }));
        ok++;
      } catch (e) {
        if (epoch !== analysisEpoch.current) return;
    ...
    if (epoch === analysisEpoch.current) {
      ...
      showToast(`本地读取完成：${ok}/${selected.length}份取得文本。候选不会自动覆盖案件字段。`, ok ? "success" : "warning");
    }
  };
  ...
  const exportDraft = () => {
    downloadBytes(JSON.stringify(makeDraft(state, materials), null, 2), "application/json", "执行申请材料助手-v1.0-本地草稿.json");
  ...
  const importDraft = async (file: File) => {
    ...
    try {
      ...
      analysisEpoch.current++; setState(next.state); setMaterials(next.materials); setAnalysisStatus("idle"); markDirty(); goTo(2);
  ...
  const download = async (kind: "docx" | "zip", includeOriginals: boolean) => {
    ...
    try {
      ...
      downloadBytes(new Uint8Array(bytes).buffer, kind === "docx" ? "application/vnd.openxmlformats-officedocument.wordprocessingml.document" : "application/zip", kind === "docx" ? "执行申请书-v1.0-核对草稿.docx" : "执行申请材料-v1.0-核对草稿包.zip");
  ...
  const reset = () => {
    ...
    analysisEpoch.current++; setState(createInitialState()); setMaterials(emptyMaterials()); setAnalysisStatus("idle"); markDirty(); goTo(1);
  };
  const statuses = {
  ...
  return (
    <AppShell currentStep={step} footerMessage={dirty ? "有未导出的修改 · 关闭前请保存草稿" : "仅页面内存 · 请确认已下载的草稿保管妥当"} onStepChange={goTo} statuses={statuses} onExportDraft={exportDraft} onImportDraft={importDraft} onReset={reset} busy={exporting || importing}>
      ...
      {step === 2 ? <CaseInfoPage state={state} materials={materials} analysisStatus={analysisStatus} onAnalyze={analyze} onMaterialChange={selectMaterial} onApplyCandidate={applyCandidate}
